  ...
  <div class="bg-white shadow-[0_4px_12px_-5px_rgba(0,0,0,0.4)] p-6 max-w-screen-xl mx-auto rounded-lg overflow-hidden">
    <div class="container mx-auto px-4 py-8">
      ...
      <div class="flex justify-between">
        <h2 class="text-2xl font-bold mb-6">Data OLT</h2>
        <div>
...
<script>
  $(document).ready(function () {
    $('#user-table').DataTable({
        ajax: {
            url: '/olt/allData', // Ganti dengan URL endpoint API Anda
            type: 'GET',
            dataSrc: '',
        },
        scrollX: true,
  ...
  function editOlt(oltId) {
     window.location.href = `/olt/edit/${oltId}`;
      // Addkan logika edit di sini
  }

  // Edit User Action
    function showOlt(oltId) {
    window.location.href = `/olt/${oltId}`;
    // Addkan logika edit di sini
    }
...
function deleteOlt(oltId) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            $.ajax({
                url: `/olt/${oltId}`, // Endpoint penghapusan
                method: 'DELETE',
                headers: {
