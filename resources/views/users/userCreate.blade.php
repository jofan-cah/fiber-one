        ...
        <div class="grid sm:grid-cols-2 gap-6">
          ...
          <div class="relative flex items-center">
            <select id="user_level_id" name="user_level_id"
              ...
              <option value="{{$item['user_level_id']}}">{{$item['user_name']}}</option>
              @endforeach
...
<script>
  $(document).ready(function() {
    $('#userForm').on('submit', function(e) {
        ...
        $.ajax({
            url: '{{ route('storeUsers') }}', // Ubah dengan route yang sesuai
            method: 'POST',
            data: formData,
            success: function(response) {
                ...
                Swal.fire({
                    ...
                    window.location.href = '/users'; // Ubah dengan route yang sesuai
                });
            },
