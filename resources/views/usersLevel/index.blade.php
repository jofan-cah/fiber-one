  ...
  <div class="bg-white shadow-[0_4px_12px_-5px_rgba(0,0,0,0.4)] p-6 max-w-screen-xl mx-auto rounded-lg overflow-hidden">
    <div class="container mx-auto px-4 py-8">
      ...
      <div class="flex justify-between">
        <h2 class="text-2xl font-bold mb-6">Data Level</h2>
        <div>

          <a href="{{ route('createUsersLevel') }}"
      ...
      <div class="overflow-x-auto">
        <table id="user-table" class="min-w-full bg-white">
          <thead class="whitespace-nowrap bg-gray-700">
            <tr>
              <th class="p-4 text-left text-sm font-semibold text-white">Level ID</th>
              <th class="p-4 text-left text-sm font-semibold text-white">Level </th>
              <th class="p-4 text-left text-sm font-semibold text-white">Level Desc</th>
...
<script>
  $(document).ready(function () {
    $('#user-table').DataTable({
        ajax: {
            url: '/level/allData', // Ganti dengan URL endpoint API Anda
            type: 'GET',
            dataSrc: '',
        },
        scrollX: true,
                    ...
                    return `
                        <div class="flex space-x-2">
                            <button class="bg-blue-500 hover:bg-blue-600 text-white py-1 px-3 rounded text-sm"
                                onclick="editUser('${row.user_level_id}')">Edit</button>
                            <button class="bg-red-500 hover:bg-red-600 text-white py-1 px-3 rounded text-sm"
                                onclick="deleteUser('${row.user_level_id}')">Delete</button>
                        </div>
                    `;
                },
            },
  ...
  function editUser(userId) {
     window.location.href = `/level/edit/${userId}`;
...
function deleteUser(userId) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            $.ajax({
                url: `/level/${userId}`, // Endpoint penghapusan
                method: 'DELETE',
                headers: {
